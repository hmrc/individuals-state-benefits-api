...
import v1.services.{EnrolmentsAuthService, MtdIdLookupService, UpdateStateBenefitService}

import scala.concurrent.{ExecutionContext, Future}

@Singleton
class UpdateStateBenefitController @Inject()(val authService: EnrolmentsAuthService,
                                             val lookupService: MtdIdLookupService,
                                             appConfig: AppConfig,
                                             requestParser: UpdateStateBenefitRequestParser,
                                             service: UpdateStateBenefitService,
                                             cc: ControllerComponents)(implicit ec: ExecutionContext)
  extends AuthorisedController(cc) with BaseController with Logging with UpdateHateoasResponses {

  implicit val endpointLogContext: EndpointLogContext =
    EndpointLogContext(
      controllerName = "UpdateStateBenefitController",
  ...
  def update(nino: String, taxYear: String, benefitId: String): Action[JsValue] =
    authorisedAction(nino).async(parse.json) { implicit request =>
      ...
      val rawData = UpdateStateBenefitRawData(
        nino = nino,
        taxYear = taxYear,
        benefitId = benefitId,
      ...
      val result =
        for {
          ...
          serviceResponse <- EitherT(service.updateStateBenefit(parsedRequest))
        } yield {
          logger.info(
            s"[${endpointLogContext.controllerName}][${endpointLogContext.endpointName}] - " +
              s"Success response received with CorrelationId: ${serviceResponse.correlationId}")

          Ok(updateStateBenefitHateoasBody(appConfig, nino, taxYear, benefitId))
