...
import v1.mocks.requestParsers.MockUpdateStateBenefitRequestParser
import v1.mocks.services.{MockAuditService, MockEnrolmentsAuthService, MockMtdIdLookupService, MockUpdateStateBenefitService}
...
class UpdateStateBenefitControllerSpec
  extends ControllerBaseSpec
    with MockEnrolmentsAuthService
    with MockMtdIdLookupService
    with MockAppConfig
    with MockUpdateStateBenefitService
    with MockUpdateStateBenefitRequestParser
  ...
  val rawData: UpdateStateBenefitRawData = UpdateStateBenefitRawData(
    nino = nino,
    taxYear = taxYear,
    benefitId = benefitId,
  ...
  val requestBody: UpdateStateBenefitRequestBody = UpdateStateBenefitRequestBody(
    startDate = "2020-04-06",
  ...
  trait Test {
    ...
    val controller = new UpdateStateBenefitController(
      authService = mockEnrolmentsAuthService,
      lookupService = mockMtdIdLookupService,
      appConfig = mockAppConfig,
      requestParser = mockUpdateStateBenefitRequestParser,
      service = mockUpdateStateBenefitService,
      cc = cc
    )
  ...
  "UpdateStateBenefitController" should {
    "return OK" when {
      "happy path" in new Test {

        MockUpdateStateBenefitRequestParser
          .parse(rawData)
          .returns(Right(requestData))

        MockUpdateStateBenefitService
          .updateStateBenefit(requestData)
